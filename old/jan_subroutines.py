#### CHANGE MODULE NAME LATER TO jan_game_base
    ...
    if "holder" in O[item]:
        ...
        if curholder != None:
            ...
            if "holding" not in O[curholder]:
                ...
                # O[curholder]["holding"].append(item)
...
def simple_command_parser_for_character(character, O, C):
    cmd = input("> ")
    while cmd not in ["Q", "Quit", "quit"]:  # OR not ending()
        cmd = cmd.split()
        cmd = [w for w in cmd if w not in C["articles"]]
        # print(f"input cmd is {cmd}")  # DEBUG
        for k,valid_cmdS in C["verbs"].items():
            # print(k, valid_cmdS)
            if any(w in cmd for w in valid_cmdS):
                ...
                if k in range(4):  # 0,1,2,3
                    if O[cur_room]["NSEW"][k] is None:
                        print(f"there is no exit to the {what_direction(k)}") #{C[k][1]}")
                    else:
